...
Workflow: lagged/rolling features, cumulative return to Friday, sqrt(DTE) standardization,
quantile buckets, one balanced logistic per bucket, convert to expected realised volatility.
"""
...
def add_weekly_friday_target(
    ...
    """Add cumulative return through Friday and DTE-standardized target."""
    out = df.copy()
    ...
    out["target_horizon_scaled_return"] = out["target_forward_return"] / np.sqrt(out["dte"])
    return out
...
def add_quantile_targets(
    ...
    bucketed, edges = pd.qcut(
        valid_target, q=n_quantiles, labels=False, retbins=True, duplicates="drop"
    )
    ...
    for q in quantile_info["quantile_category"]:
        out[f"quantile_{q}_dummy"] = out["quantile_category"].eq(q).fillna(False).astype(int)
...
def fit_balanced_bucket_models(
    ...
    for q in quantile_info["quantile_category"]:
        ...
        model = make_pipeline(
            StandardScaler(),
            LogisticRegression(class_weight="balanced", max_iter=2000, solver="lbfgs", random_state=42),
        )
        model.fit(X, y)
        models[int(q)] = model
...
def compute_expected_realised_volatility(
    ...
    out["expected_scaled_return_sq_plain"] = np.where(prob_mass > 0, expected_scaled_return_sq_plain, np.nan)
    out["expected_scaled_return_sq"] = np.where(prob_mass > 0, expected_scaled_return_sq_weighted, np.nan)
...
def run_logistic_expected_vol(
    ...
    modeled, quantile_info = add_quantile_targets(
        modeled, target_col="target_horizon_scaled_return", n_quantiles=n_quantiles
    )
    feature_cols = [
        c for c in modeled.columns
        if c.startswith(f"{return_col}_lag_") or c.startswith(f"abs_{return_col}_lag_")
        or c.startswith("rolling_") or c in {"weekday", "dte"}
    ]
    modeled, models = fit_balanced_bucket_models(modeled, feature_cols=feature_cols, quantile_info=quantile_info)
...
def run_logistic_expected_vol_oos(
    ...
    for t in range(train_window, n_rows):
        ...
        train_labeled, quantile_info = add_quantile_targets(
            train_df, target_col="target_horizon_scaled_return", n_quantiles=n_quantiles
        )
        train_labeled, models = fit_balanced_bucket_models(
            train_labeled, feature_cols=feature_cols, quantile_info=quantile_info
        )
        ...
        for q in quantile_info["quantile_category"]:
            ...
            raw_probs[int(q)] = float(model.predict_proba(X_test)[:, 1][0]) if model else 0.0
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Balanced logistic bucket model for Friday-expiry return distribution.")
    parser.add_argument("--input", type=Path, default=DATA_OUTPUT_DIR / "feature_table.csv", help="Feature table CSV.")
    parser.add_argument("--output", type=Path, default=DATA_OUTPUT_DIR / "logistic_expected_rvol.csv", help="Output CSV.")
...
def main() -> None:
    ...
    modeled, quantile_info, _ = run_logistic_expected_vol(
        feature_df, return_col="log_return", n_quantiles=args.quantiles
    )
    ...
    preview_cols = [
        "date", "log_return", "weekday", "days_to_friday", "target_forward_return",
        "target_horizon_scaled_return", "target_value", "quantile_category",
        "expected_scaled_return_sq", "expected_realised_volatility", "expected_realised_variance",
    ]
    print(modeled[preview_cols].dropna().head().to_string(index=False))
    print(f"\nSaved to {args.output} and {quantile_output}")
