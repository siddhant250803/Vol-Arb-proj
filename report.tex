...
\author{
    ...
    Stanford University \\
     \href{https://github.com/siddhant250803/Vol-Arb-proj}{https://github.com/siddhant250803/Vol-Arb-proj}
}
\date{Winter 2026}
...
We trade \textbf{delta-hedged ATM straddles on SPX weekly options}, entering positions whenever the spread between implied volatility and our forecast of realized volatility (RV) is statistically extreme.
...
\begin{enumerate}[nosep]
    ...
    \item \textbf{Variance Swap Signal}: Replaces ATM IV with model-free implied variance (MFIV), computed via the discrete Carr--Madan integration \cite{carr1999option} over out-of-the-money option prices.
...
Rather than forecasting realised volatility (RV) directly, this approach first models the distribution of the underlying asset's log-return till expiry. Once this return distribution is estimated, it can be converted into an implied forecast of weekly RV. Since the options data are weekly and expire each Friday, the target variable must be normalised so that observations from different days of the week are comparable.

\begin{enumerate}
    \item Construct \(K\) quantile buckets for the underlying’s log-return from the current day until expiry. To make returns comparable across the week, normalise each log-return by dividing by \(\sqrt{\text{days to expiry}}\).
...
\end{equation}
where $\bar{\mu}_{252}$ and $\bar{\sigma}_{252}$ are the backward rolling 252-day mean and standard deviation of the VRP series.
...
Accordingly, we obtain daily SPX closes from Yahoo Finance (ticker \texttt{\^{}GSPC40}) over the full sample window, with an additional one-year buffer on each side to support rolling-window model estimation.
...
\paragraph{Secondary Backtest}
We partition the signal dates chronologically: the first 60\% form the in-sample training period (April 2011 to July 2018), and the remaining 40\% constitute the out-of-sample test period (July 2018 to February 2023). This serves to test whether the signal decays over the training sample.
...
A passive buy-and-hold position in the SPX over the same period yields a Sharpe ratio of 0.64, an annualized return of 10.35\%, and a maximum drawdown of $-$33.92\%.

The VRP strategy exhibits an annualized Jensen's alpha of 202.31\%, a market beta of 0.136, and an information ratio of 1.023. The near-zero correlation with the equity market (0.013) indicates that returns primarily derive from the volatility risk premium rather than directional equity exposure.

The strategy produces positive risk-adjusted returns with a Sharpe ratio of 1.10. The positive skewness (1.43) is unusual for a short-volatility strategy and reflects the contribution of profitable long-volatility trades during stress periods.

The maximum drawdown of 83.9\% highlights the substantial tail risk inherent in volatility selling strategies and underscores the importance of the stop-loss mechanism.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{07_cumulative_pnl.png}
    \caption{Cumulative PnL, equity curve and drawdown for the VRP strategy (2011--2023).}
    \label{fig:cumulative_pnl}
\end{figure}
\subsection{Strategy Comparison}

The Table below and Figure~\ref{fig:comparison} present a head-to-head comparison of the three signal families.
...
\begin{tabular}{@{}lrrr@{}}
\toprule
\textbf{Metric} & \textbf{VRP} & \textbf{Var Swap} & \textbf{Log ($K{=}6$)} \\
\midrule
Trades           & 140 & 151 & 135 \\
...
The variance swap signal generates higher total PnL (\$37.21\,M versus \$26.04\,M) and a higher Sharpe ratio (1.26 versus 1.10) than the ATM IV baseline. This improvement is consistent with the theoretical advantage of model-free implied variance, which captures the full distribution of risk-neutral expectations rather than relying on a single at-the-money volatility measure.

The logistic regression RV forecast, optimized at $K{=}6$ quantile buckets, achieves the highest Sharpe ratio (1.32) and a cumulative PnL of \$26.18\,M, with a win rate of 54\%. The lower annualized volatility relative to the variance swap strategy also contributes to improved risk-adjusted performance.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{13_strategy_comparison.png}
...
Tail events such as the February 2018 ``Volmageddon'' episode~\citep{augustin2021volmageddon} can generate severe losses for short-volatility strategies due to feedback effects from hedge rebalancing and forced deleveraging. To mitigate this risk, we impose a 25\% stop-loss rule. The straddle is marked to market using realized volatility accumulated over the trade, allowing the stop to trigger promptly when volatility spikes. Losses are capped at 25\% of the initial premium deployed on every exit, including expiry. During the Volmageddon stress window, the strategy executes 7 trades and produces $4.74$ M in PnL. The full-sample maximum drawdown is 83.9\% .

\subsection{Secondary Backtest Results}
Using a 60/40 chronological split at July 2018, the in-sample period (June 2011--July 2018) contains 87 trades with a 54.0\% win rate, generating \$19.78\,M in cumulative PnL, a Sharpe ratio of 1.69, and a Sortino ratio of 20.80, with a maximum drawdown of $-$83.9\%.

The out-of-sample period (August 2018--February 2023) contains 53 trades with a 49.1\% win rate, producing \$6.27\,M in cumulative PnL, a Sharpe ratio of 0.48, and a Sortino ratio of 4.85, with a maximum drawdown of $-$79.8\%.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{15_oos_walkforward.png}
...
The strategy generates positive cumulative PnL in all three volatility regimes, although performance varies across environments. The high-volatility tercile delivers the strongest risk-adjusted performance, with a Sharpe ratio of 0.62 and a win rate of 55\%. The low-volatility tercile produces the largest cumulative profit ($16.07$ M) despite a lower Sharpe ratio of 0.57, reflecting a larger number of trades. By contrast, the medium-volatility tercile exhibits substantially weaker performance, with a Sharpe ratio of 0.06 and a win rate of 37%.

Overall, the strategy does not exhibit a volatility regime in which it becomes systematically unprofitable. This pattern is consistent with the view that the variance risk premium is a persistent feature of SPX option markets rather than a phenomenon confined to specific volatility environments, in line with the equilibrium arguments of \citet{drechsler2011whats}.
...
A threshold of $z > 0.50$ yields a Sharpe of 1.34 with 295 trades; $z > 1.00$ produces a Sharpe of 1.10 with 158 trades; and $z > 2.00$ produces a Sharpe of 0.18 with 39 trades.
...
The sample Sharpe ratio is 1.10, while the bootstrap distribution yields a mean Sharpe of 1.18. The 95\% bootstrap confidence interval is $[0.38,\,2.48]$, and the estimated probability that the Sharpe ratio exceeds zero is 100.0\%. The entire 95\% confidence interval lies above zero, providing strong statistical evidence that the observed performance is unlikely to be attributable to sampling variation alone (Figure~\ref{fig:bootstrap}).
...
Our core VRP signal, combined with a 25\% per-trade stop-loss, produces a Sharpe ratio of 1.10 over the 2011--2023 sample period, generating \$26.04\,M in cumulative PnL across 140 trades on \$1\,M notional trade size.
...
\begin{itemize}[nosep]
    ...
    \item The logistic regression RV forecast (optimized at $K{=}6$ quantile buckets) achieves a Sharpe ratio of 1.32 and \$26.18\,M in cumulative PnL.
