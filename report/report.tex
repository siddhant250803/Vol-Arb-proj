...
On \$1M notional capital over 2011--2023, the core VRP strategy (with a 25\% stop-loss per trade) generates 93 trades with a Sharpe ratio of 2.55, a 50.5\% win rate, and \$7.63M cumulative PnL.
The strategy is profitable out-of-sample and across most stress periods; a per-trade stop-loss (25\% of trade value) limits tail losses so that maximum drawdown is 81.5\% rather than the 98\%+ that would occur without it (e.g., the February 2018 ``Volmageddon'' event~\citep{augustin2021volmageddon}).
...
Profit Factor & 1.87 \\
Annualized Return & 110.4\% \\
Annualized Volatility & 43.2\% \\
Sharpe Ratio & 2.55 \\
Sortino Ratio & 2.25 \\
Calmar Ratio & 1.35 \\
Maximum Drawdown & $-$81.5\% \\
Return Skewness & $-$4.96 \\
Return Kurtosis & 46.5 \\
VaR (95\%) & $-$1.66\% daily \\
CVaR (95\%) & $-$5.40\% daily \\
\bottomrule
\end{tabular}
...
The strategy generates strong risk-adjusted returns (Sharpe 2.55) with a 25\% stop-loss limiting per-trade losses: the average winner (\$348K) exceeds the average loser (\$190K), and 24 trades exit via stop-loss so tail events are capped.
Negative skewness ($-$4.96) and high kurtosis (46.5) reflect remaining left-tail risk, but maximum drawdown is 81.5\% rather than 98\%+ without the stop.
...
\midrule
Trades           & 93 (24 stop) & 78    & 85    \\
Win Rate         & 50.5\% & 62.8\% & 47.1\% \\
Total PnL        & \$7.63M & \$5.62M & $-$\$1.73M \\
Ann.\ Return     & 110.4\% & 95.4\% & $-$25.5\% \\
Ann.\ Volatility & 43.2\% & 53.2\% & 34.3\% \\
Sharpe           & 2.55  & 1.79  & $-$0.74 \\
Sortino          & 2.25  & 1.77  & $-$0.96 \\
Max Drawdown     & $-$81.5\% & $-$98.6\% & $-$98.1\% \\
\bottomrule
\end{tabular}
\end{table}

The VRP strategy (with 25\% stop-loss) generates higher total PnL and a higher Sharpe (2.55 vs.\ 1.79 for Var Swap) with a lower max drawdown (81.5\% vs.\ 98.6\%).
...
The Volmageddon-style trade is therefore exited early and books at most a 25\% loss on the position instead of 300\%+ of capital, and maximum drawdown falls to $-$81.5\%.
...
Sharpe         & 1.40        & 7.05        \\
Sortino        & 1.19        & 17.21       \\
Max Drawdown   & $-$81.5\%   & (see full sample) \\
\bottomrule
\end{tabular}
\end{table}
...
Our baseline $z > 1.0$ lies in the robust middle range (Sharpe 2.55 with stop-loss).
...
Sharpe degrades only slightly as costs rise (e.g., 2.55 at 5 bps), indicating that the volatility premium far exceeds realistic trading frictions for SPX options.
...
\begin{itemize}[nosep]
    \item Sample Sharpe: 2.55 (with 25\% stop-loss)
...
Our core VRP signal with a 25\% per-trade stop-loss produces a Sharpe ratio of 2.55 over 2011--2023 with 93 trades (24 stop-loss exits) on \$1M notional and \$7.63M cumulative PnL.
...
\begin{enumerate}[nosep]
    ...
    \item The strategy thrives during most crises; a 25\% stop-loss (of trade value) limits losses in Volmageddon-type events so maximum drawdown is 81.5\% instead of 98\%+.
...
\paragraph{Tail Risk.}
Without a stop-loss, a single trade (e.g., Volmageddon) can drive drawdown to 98\%; with a 25\% stop per trade (and MTM using realized vol, loss capped on exit), maximum drawdown is 81.5\%.
