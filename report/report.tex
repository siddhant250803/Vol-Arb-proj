...
\graphicspath{{report/figures/}{../output/figures/}}
...
On \$1M notional capital over 2011--2023, the core VRP strategy (with a 25\% stop-loss per trade) generates 93 trades with a Sharpe ratio of 5.26, a 50.5\% win rate, and \$7.63M cumulative PnL.
The strategy is profitable out-of-sample and across most stress periods; a per-trade stop-loss (25\% of trade value) limits tail losses so that maximum drawdown is 79.4\% rather than the 98\%+ that would occur without it (e.g., the February 2018 ``Volmageddon'' event~\citep{augustin2021volmageddon}).
...
The primary backtest runs over the full signal period (April 2011 -- February 2023; 2,782 signal days, 93 trades with 25\% stop-loss).
...
Trades/Year & 8.0 \\
Win Rate & 50.5\% \\
Total Net PnL & \$7,625,207 \\
Average Trade PnL & \$81,991 \\
Median Trade PnL & \$1,939 \\
Profit Factor & 1.87 \\
Annualized Return & 165.2\% \\
Annualized Volatility & 31.4\% \\
Sharpe Ratio & 5.26 \\
Sortino Ratio & 6.46 \\
Calmar Ratio & 2.08 \\
Maximum Drawdown & $-$79.4\% \\
Return Skewness & $-$1.19 \\
Return Kurtosis & 11.7 \\
VaR (95\%) & $-$1.56\% daily \\
CVaR (95\%) & $-$3.56\% daily \\
\bottomrule
\end{tabular}
...
\noindent\textbf{Benchmark (buy-and-hold SPX):} Sharpe 0.62, ann.\ return 10.9\%, max drawdown $-$33.0\%. Strategy alpha (ann.) 88.5\%, information ratio 2.44. Capacity (1\% of SPX options ADV): \$500M.

The strategy generates strong risk-adjusted returns (Sharpe 5.26) with a 25\% stop-loss limiting per-trade losses: the average winner (\$348K) exceeds the average loser (\$190K).
Negative skewness ($-$1.19) and elevated kurtosis (11.7) reflect some left-tail risk, but maximum drawdown is 79.4\% rather than 98\%+ without the stop.
...
\midrule
Trades           & 93 & 78    & 85    \\
Win Rate         & 50.5\% & 62.8\% & 47.1\% \\
Total PnL        & \$7.63M & \$5.62M & $-$\$1.73M \\
Ann.\ Return     & 165.2\% & 95.4\% & $-$25.5\% \\
Ann.\ Volatility & 31.4\% & 53.2\% & 34.3\% \\
Sharpe           & 5.26  & 1.79  & $-$0.74 \\
Sortino          & 6.46  & 1.77  & $-$0.96 \\
Max Drawdown     & $-$79.4\% & $-$98.6\% & $-$98.1\% \\
\bottomrule
\end{tabular}
\end{table}

The VRP strategy (with 25\% stop-loss) generates higher total PnL and a higher Sharpe (5.26 vs.\ 1.79 for Var Swap) with a lower max drawdown (79.4\% vs.\ 98.6\%).
...
The Volmageddon-style trade is therefore exited early and books at most a 25\% loss on the position instead of 300\%+ of capital, and maximum drawdown falls to $-$79.4\%.
...
Sharpe         & 1.40        & 7.05        \\
Sortino        & 1.19        & 17.21       \\
Max Drawdown   & $-$79.4\%   & (see full sample) \\
\bottomrule
\end{tabular}
\end{table}

The strategy performs \emph{better} out-of-sample than in-sample.
With the stop-loss, the Volmageddon event is contained in-sample; OOS performance remains strong with lower leverage in the later period as SPX levels rose.
...
Volmageddon (2018)       & 4 & 50\% & capped (max $-$\$315K) & $-$0.84 \\
...
With the 25\% stop-loss, Volmageddon-period losses are capped so the episode no longer dominates the equity curve; the premium remains exploitable except during sudden volatility explosions, which the stop now limits.
...
Our baseline $z > 1.0$ lies in the robust middle range (Sharpe 5.26 with stop-loss).
...
Sharpe degrades only slightly as costs rise (e.g., 5.26 at 5 bps), indicating that the volatility premium far exceeds realistic trading frictions for SPX options.
...
\begin{itemize}[nosep]
    \item Sample Sharpe: 5.26 (with 25\% stop-loss)
    \item Bootstrap analysis yields a wide CI due to fat-tailed returns; $P(\text{Sharpe} > 0)$ remains high.
\end{itemize}

The stop-loss reduces the impact of extreme losses (e.g., Volmageddon) on the equity curve, so the observed Sharpe and max drawdown reflect the risk-managed strategy.
...
With the stop-loss, 2018 no longer shows a single catastrophic loss; losses in stress years (e.g., 2012, 2015, 2020) are limited by the per-trade cap, and cumulative PnL is \$7.63M.
...
Our core VRP signal with a 25\% per-trade stop-loss produces a Sharpe ratio of 5.26 over 2011--2023 with 93 trades on \$1M notional and \$7.63M cumulative PnL.
...
\begin{enumerate}[nosep]
    ...
    \item The strategy thrives during most crises; a 25\% stop-loss (of trade value) limits losses in Volmageddon-type events so maximum drawdown is 79.4\% instead of 98\%+.
...
\paragraph{Tail Risk.}
Without a stop-loss, a single trade (e.g., Volmageddon) can drive drawdown to 98\%; with a 25\% stop per trade (and MTM using realized vol, loss capped on exit), maximum drawdown is 79.4\%.
