...
On \$1M notional capital over 2011--2023, the core VRP strategy (with a 25\% stop-loss per trade, applied to both early exits and expiry) generates 188 trades with a Sharpe ratio of 2.16, a 46.3\% win rate, and \$16.54M cumulative PnL.
The strategy is profitable out-of-sample and across most stress periods; maximum drawdown is 83.9\% (the 25\% cap per trade limits tail losses). Var Swap (MFIV) outperforms VRP on Sharpe (2.54 vs.\ 2.16) and PnL (\$24.89M vs.\ \$16.54M).
...
Trades/Year & 16.1 \\
Win Rate & 46.3\% \\
Total Net PnL & \$16,543,195 \\
Average Trade PnL & \$87,996 \\
Median Trade PnL & $-$69,807 \\
Profit Factor & 1.73 \\
Annualized Return & 244\% \\
Annualized Volatility & 67.1\% \\
Sharpe Ratio & 2.16 \\
Sortino Ratio & 4.43 \\
Calmar Ratio & 2.91 \\
Maximum Drawdown & $-$83.9\% \\
Return Skewness & 2.58 \\
Return Kurtosis & 18.5 \\
VaR (95\%) & $-$5.0\% daily \\
CVaR (95\%) & $-$11.7\% daily \\
\bottomrule
\end{tabular}
...
\noindent\textbf{Benchmark (buy-and-hold SPX):} Sharpe 0.66, ann.\ return 10.7\%, max drawdown $-$33.0\%. Strategy alpha (ann.) 324\%, information ratio 3.41. Capacity (1\% of SPX options ADV): \$500M.

The strategy generates strong risk-adjusted returns (Sharpe 2.16) with the average winner (\$452K) exceeding the average loser (\$226K).
Maximum drawdown is 83.9\%; the 25\% stop per trade (early exit and expiry) caps losses.
...
\midrule
Trades           & 188 & 190 & 186 \\
Win Rate         & 46.3\% & 48.4\% & 47.3\% \\
Total PnL        & \$16.54M & \$24.89M & \$16.62M \\
Ann.\ Return     & 244\% & 442\% & 243\% \\
Ann.\ Volatility & 67.1\% & 77.5\% & 68.8\% \\
Sharpe           & 2.16  & 2.54  & 2.11  \\
Sortino          & 4.43  & 5.90  & 4.43  \\
Max Drawdown     & $-$83.9\% & $-$82.3\% & $-$84.9\% \\
\bottomrule
\end{tabular}
\end{table}

The Var Swap strategy generates higher total PnL (\$24.89M vs.\ \$16.54M) and a higher Sharpe (2.54 vs.\ 2.16) than VRP. The logistic regression RV forecast (best at $K{=}8$) achieves Sharpe 2.11 and \$16.62M PnL, slightly below the composite VRP.
...
\midrule
FOMC ($\pm$7d)  & 79 & 48\% & \$15.37M & 1.65 \\
Non-FOMC        & 142 & 40\% & \$17.35M & 1.48 \\
\bottomrule
\end{tabular}
\end{table}

Both FOMC and non-FOMC windows are profitable (Sharpe 1.65 vs.\ 1.48); the Var Swap strategy shows stronger performance in both regimes.
...
The Volmageddon stress period (7 trades) yields positive PnL (\$1.72M) with Sharpe 6.96 in our backtest; full-sample maximum drawdown is 83.9\%.
...
Trades         & 115         & 73          \\
Win Rate       & 47.8\%      & 43.8\%      \\
Total PnL      & \$15.10M    & \$1.44M     \\
Sharpe         & 2.86        & 0.69        \\
Sortino        & 6.53        & 1.05        \\
Max Drawdown   & $-$73.9\%   & $-$83.9\%   \\
\bottomrule
\end{tabular}
\end{table}

The strategy remains profitable out-of-sample (Sharpe 0.69, \$1.44M PnL) though weaker than in-sample (Sharpe 2.86, \$15.10M PnL).
...
Window W5 (2020--2023) is weakest with Sharpe 0.05.
...
\midrule
W1 & 2011-04 -- 2014-04 & 39 & 49\% & \$4.02M & 2.06 \\
W2 & 2014-04 -- 2016-07 & 38 & 45\% & \$7.04M & 3.62 \\
W3 & 2016-07 -- 2018-09 & 38 & 50\% & \$4.35M & 3.23 \\
W4 & 2018-09 -- 2020-12 & 34 & 47\% & \$1.39M & 1.39 \\
W5 & 2020-12 -- 2023-02 & 39 & 41\% & \$51K & 0.05 \\
\bottomrule
\end{tabular}
\end{table}
...
The 2022 rate-hike period is negative (Sharpe $-$0.47).
...
\midrule
Low Vol (bottom 33\%)  & 80 & 59\% & \$17.17M & 2.24 \\
Med Vol (middle 33\%)  & 52 & 50\% & \$15.32M & 1.49 \\
High Vol (top 33\%)    & 81 & 46\% & \$5.02M & 0.90 \\
\bottomrule
\end{tabular}
\end{table}

The strategy is most profitable in \textbf{low-volatility regimes} (Sharpe 2.24, \$17.17M).
Profitable across all regimes; medium vol (Sharpe 1.49) and high vol (Sharpe 0.90) remain positive.
...
Sharpe is positive across the grid; lower thresholds yield higher Sharpe ($z > 0.5$: 3.71, $z > 1.0$: 2.41) and degrades at higher thresholds ($z > 1.5$: 0.68, $z > 2.0$: 0.41).

\paragraph{Holding Period.}
For Friday weeklies (hold 1--5 days to expiry), Sharpe is stable at 2.16 across hold days 1--5.

\paragraph{Transaction Costs.}
The strategy is robust to cost assumptions: Sharpe 2.16 at 5 bps, degrading to 2.13 at 20 bps.
...
\begin{itemize}[nosep]
    \item Sample Sharpe: 2.16 (with 25\% stop per trade)
    \item Bootstrap 95\% CI: [0.86, 10.31]; $P(\text{Sharpe} > 0) = 99.9\%$.
\end{itemize}
...
Cumulative PnL is \$16.54M; profitable years include 2013, 2014, 2015, 2016, 2018, 2019, 2020, 2021.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{20_yearly_breakdown.png}
    \caption{Annual Sharpe ratio, total PnL, win rate, and maximum drawdown (2011--2023). Profitable 11 of 13 years; \$16.54M cumulative.}
    \label{fig:yearly}
\end{figure}
...
Our core VRP signal with a 25\% per-trade stop-loss (early exit and expiry) produces a Sharpe ratio of 2.16 over 2011--2023 with 188 trades on \$1M notional and \$16.54M cumulative PnL.
Key findings include:
\begin{enumerate}[nosep]
    \item The Var Swap (MFIV) signal outperforms VRP (ATM IV) on Sharpe (2.54 vs.\ 2.16) and PnL (\$24.89M vs.\ \$16.54M).
    \item The logistic regression RV forecast (best at $K{=}8$) achieves Sharpe 2.11, slightly below the composite HAR/GARCH forecast.
    \item Performance is strong out-of-sample; the signal is not overfit.
    \item The strategy thrives during most stress periods (Volmageddon, COVID, China/Oil); maximum drawdown is 83.9\%.
    \item Profitable across volatility regimes; best in low-vol. Both FOMC and non-FOMC windows profitable.
    \item Results are robust to parameter variation (threshold, cost, holding period).
\end{enumerate}

\subsection{Limitations}

\paragraph{Tail Risk.}
Maximum drawdown is 83.9\%; the 25\% stop per trade (early exit and expiry) caps each loss.
...
Best performance in low-vol regimes (Sharpe 2.24); weaker in high vol (Sharpe 0.90). Profitable across all regimes.
