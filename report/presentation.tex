  ...
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      ...
      \begin{itemize}
        ...
        \item Hold to expiry (1--5d for weeklies), 25\% stop-loss, \$1M notional, 5\,bps cost.
      \end{itemize}
    \end{column}
    \begin{column}{0.48\textwidth}
...
\begin{frame}{Monthly Returns \& Key Metrics}
  ...
  \footnotesize
  \textbf{Full-sample (VRP, 25\% stop):} 188 trades, 46.3\% win, \$8.91M total PnL; Sharpe 0.90, Sortino 1.22, Calmar 0.35; Ann.\ return 35.1\%, vol 86.9\%, max DD $-$99.8\%; Benchmark (SPX): Sharpe 0.66; alpha 117.9\%, info ratio 0.31.
\end{frame}
...
\begin{frame}{Strategy Comparison}
  ...
  \footnotesize
  \textbf{Var Swap leads:} Var Swap: 190 trades, Sharpe 1.06, \$14.04M, max DD $-$100\%; VRP: 188 trades, Sharpe 0.90, \$8.91M, max DD $-$99.8\%. Both show high drawdowns.
\end{frame}
...
\begin{frame}{Results Comparison: All Strategies}
  ...
  \begin{tabular}{@{}lrrrrr@{}}
    ...
    \midrule
    VRP (ATM IV, composite RV) & 188 & 46.3\% & 0.90 & \$8.91M & $-$99.8\% \\
    Var Swap (MFIV) & 190 & 48.4\% & 1.06 & \$14.04M & $-$100.0\% \\
    \bottomrule
  \end{tabular}
  \vspace{0.5em}
  \normalsize
  \raggedright
  \footnotesize
  Same backtest: delta-hedged straddles, \$1M notional, hold to expiry (1--5d), 25\% stop, 5\,bps cost.
\end{frame}
...
\begin{frame}{FOMC Window Analysis}
  ...
  \footnotesize
  \textbf{FOMC vs non-FOMC (VRP):} FOMC: 79 trades, 48\% win, $-$\$3.13M, Sharpe $-$0.19; Non-FOMC: 142 trades, 40\% win, \$2.78M, Sharpe 0.18. VRP weaker in FOMC windows.
\end{frame}
...
\begin{frame}{Out-of-Sample \& Walk-Forward}
  ...
  \footnotesize
  \textbf{OOS (60/40 split):} In-sample: 115 trades, Sharpe 1.11, \$7.96M; Out-of-sample: 73 trades, Sharpe 0.45, \$956K. \textbf{Walk-forward:} 4 of 5 windows profitable; W5 (2020--23) negative.
\end{frame}
...
\begin{frame}{Volatility Regime}
  ...
  \footnotesize
  \textbf{Regime (VRP):} Low vol: \$9.13M, Sharpe 0.79; Medium vol: $-$\$1.21M, Sharpe $-$0.06; High vol: $-$\$5.71M, Sharpe $-$0.26. Most profitable in low-vol regimes.
\end{frame}
...
\begin{frame}{Parameter Sensitivity (Robustness)}
  ...
  \footnotesize
  \textbf{Findings:} Z-threshold: Sharpe positive at $z \leq 1.25$; degrades at $z > 1.5$. Hold: stable at 0.90 for 1--5d (weeklies). Cost: robust 0--20\,bps.
\end{frame}
...
\begin{frame}{Bootstrap Sharpe \& Yearly Breakdown}
  \begin{columns}[T]
    ...
    \begin{column}{0.48\textwidth}
      ...
      \end{adjustbox}
      \vspace{-0.3em}
      \scriptsize Annual Sharpe, PnL, win rate, max DD (2011--2023). Profitable 6 of 13 years; \$8.91M cumulative.
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{Summary}
  \begin{itemize}
    ...
    \item Sharpe 0.90, \$8.91M PnL (188 trades); Var Swap: Sharpe 1.06, \$14.04M (190 trades). Max DD $\approx$ 100\% for both.
...
\begin{frame}{Limitations}
  \begin{itemize}
    \item Tail risk: $\approx$100\% max DD; stop assumes executable at cap.
